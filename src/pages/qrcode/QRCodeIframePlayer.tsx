...
const QRCodeReader = () => {
  ...
  useEffect(() => {
    QrScanner.listCameras(true).then((devices) => {
      // tenta filtrar pela câmera traseira ("back" ou "environment")
      ...
      const mapped = (backCameras.length > 0 ? backCameras : devices).map(
        (d) => ({
          id: d.id,
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      const brVoices = allVoices.filter((v) =>
        v.lang.toLowerCase().includes("pt-br")
      );
      setVoices(brVoices);
      if (brVoices.length > 0) setSelectedVoice(brVoices[0]);
    };
    ...
    <div className="flex flex-col items-center gap-4 mt-28 p-4">
      ...
      {qrResult && (
        <div className="relative w-80 h-[28rem] bg-[#e4cfa3] border-4 border-yellow-700 rounded-xl shadow-2xl flex flex-col">
          ...
          <div className="flex justify-between items-center px-3 py-1 text-[0.7rem] font-bold text-black border-t border-yellow-700"></div>
